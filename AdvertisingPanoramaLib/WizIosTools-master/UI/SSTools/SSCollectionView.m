...
- (NSUInteger) numberOfItemsInSection:(NSUInteger)section  update:(BOOL)update
{
...
- (void)selectItemAtIndexPath:(NSIndexPath *)indexPath animated:(BOOL)animated scrollPosition:(SSCollectionViewScrollPosition)scrollPosition {
    ...
    if (indexPath) {
        ...
        if ([self.delegate respondsToSelector:@selector(collectionView:didSelectItemAtIndexPath:item:)]) {
            [self.delegate collectionView:self didSelectItemAtIndexPath:indexPath item:item];
...
- (void)endUpdates {
	...
	for (NSNumber *key in sections) {
		...
		NSInteger totalItems = [self numberOfItemsInSection:sectionIndex update:NO];
...
- (NSUInteger)_numberOfRowsInSection:(NSUInteger)section {
	...
	NSUInteger totalItems = [self numberOfItemsInSection:section update:YES];
...
- (NSArray *)_itemsForRowIndexPath:(NSIndexPath *)rowIndexPath {
	NSUInteger totalItems = [self numberOfItemsInSection:rowIndexPath.section update:YES];
...
- (void) reloadSections:(NSIndexSet *)sections withRowAnimation:(SSCollectionViewItemAnimation)animation
{
    [self invalidateSectionCache];
    [_tableView reloadSections:sections withRowAnimation:(UITableViewRowAnimation)animation];
}
@end
