...
@implementation MWPhotoBrowser{
    ...
    UIButton* copyPhotoButton;
    UIButton* emailPhotoButton;
    UILabel*  pageInfoLabel;
}
...
- (void)viewDidLoad {
    ...
    savePhotoButton = [UIButton buttonWithType:UIButtonTypeCustom];
    if (iPad) {
        [savePhotoButton setFrame:CGRectMake(30, CGRectGetHeight(self.view.frame) - 54, 44, 44)];
    }else{
        [savePhotoButton setFrame:CGRectMake(12, CGRectGetHeight(self.view.frame) - 54, 44, 44)];
    }
    [savePhotoButton setImage:WizImageByKind(ImageOfViewPhotoSaveIcon) forState:UIControlStateNormal];
    [savePhotoButton addTarget:self action:@selector(savePhoto) forControlEvents:UIControlEventTouchUpInside];
    savePhotoButton.autoresizingMask = UIViewAutoresizingFlexibleTopMargin;
    [self.view addSubview:savePhotoButton];
    
    copyPhotoButton = [UIButton buttonWithType:UIButtonTypeCustom];
    if (iPad) {
        [copyPhotoButton setFrame:CGRectMake(CGRectGetMaxX(savePhotoButton.frame) + 30, CGRectGetMinY(savePhotoButton.frame), 44, 44)];
    }else{
        [copyPhotoButton setFrame:CGRectMake(CGRectGetMaxX(savePhotoButton.frame) + 20, CGRectGetMinY(savePhotoButton.frame), 44, 44)];
    }
    
    [copyPhotoButton setImage:WizImageByKind(ImageOfViewPhotoCopyIcon) forState:UIControlStateNormal];
    [copyPhotoButton addTarget:self action:@selector(copyPhoto) forControlEvents:UIControlEventTouchUpInside];
    copyPhotoButton.autoresizingMask = UIViewAutoresizingFlexibleTopMargin;
    [self.view addSubview:copyPhotoButton];
    
    emailPhotoButton = [UIButton buttonWithType:UIButtonTypeCustom];
    if (iPad) {
        [emailPhotoButton setFrame:CGRectMake(CGRectGetWidth(self.view.frame) - 30 - 44, CGRectGetMinY(savePhotoButton.frame), 44, 44)];
    }else{
        [emailPhotoButton setFrame:CGRectMake(CGRectGetWidth(self.view.frame) - 56, CGRectGetMinY(savePhotoButton.frame), 44, 44)];
    }
    [emailPhotoButton setImage:WizImageByKind(ImageOfViewPhotoShareIcon) forState:UIControlStateNormal];
    [emailPhotoButton addTarget:self action:@selector(emailPhoto) forControlEvents:UIControlEventTouchUpInside];
    emailPhotoButton.autoresizingMask = UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleLeftMargin;
    [self.view addSubview:emailPhotoButton];
    
    pageInfoLabel = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMaxX(copyPhotoButton.frame) + 20, CGRectGetMinY(copyPhotoButton.frame), CGRectGetMinX(emailPhotoButton.frame) - CGRectGetMaxX(copyPhotoButton.frame) - 40, 44)];
    pageInfoLabel.autoresizingMask = UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleTopMargin;
    [self.view addSubview:pageInfoLabel];
    pageInfoLabel.textColor = [UIColor whiteColor];
    pageInfoLabel.backgroundColor = [UIColor clearColor];
    pageInfoLabel.textAlignment = UITextAlignmentCenter;
    pageInfoLabel.text = [NSString stringWithFormat:@"%d/%d",_currentPageIndex + 1,[self numberOfPhotos]];
}
...
- (void)toggleControls { [self setControlsHidden:YES animated:YES permanent:YES]; }

- (void)dismissThisControls{
    ...
    if (photo) {
        ...
        if (mailClass != nil){
            if ([mailClass canSendMail]){
                ...
                [emailer addAttachmentData:UIImagePNGRepresentation([photo underlyingImage]) mimeType:@"image/png" fileName:@"Photo.png"];
                if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
