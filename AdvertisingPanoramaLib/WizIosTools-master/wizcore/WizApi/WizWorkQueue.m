...
static NSString* const WizDocDisplayModelWorkQueue = @"WizDocDisplayModelWorkQueue";

static NSString* const WizDownloadSourceWorkQueue = @"WizDownloadSourceWorkQueue";

void (^WizAddAbstractWork)(WizGenerateAbstractType ,NSString*, NSString*kbguid, NSString*) = ^(WizGenerateAbstractType type,NSString* docguid, NSString* kbguid, NSString* accountUserId)
{
...
void(^WizAddGenerateAbstractWork)(NSString*, NSString*, NSString*) = ^(NSString* docguid, NSString* kbguid, NSString* accountUserId) {
    WizAddAbstractWork(WizGenerateAbstractTypeReload, docguid, kbguid, accountUserId);
};

void(^WizAddDeleteAbstractWork)(NSString*, NSString*, NSString*) = ^(NSString* docguid, NSString* kbguid, NSString* accountUserId) {
    WizAddAbstractWork(WizGenerateAbstractTypeDelete, docguid, kbguid, accountUserId);
};
...
- (NSString*) key
{
    return [NSString stringWithFormat:@"%@%@%@",self.docGuid, self.kbguid, self.accountUserId];
}
...
+ (WizWorkQueue*) messageCountQueue
{
    static WizWorkQueue* workQueue = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        workQueue =  [WizGlobalData shareInstanceFor:[WizWorkQueue class] category:@"messageCountQueeue"];
    });
    return workQueue;
...
+ (WizWorkQueue*) docDisplayModelQueue
{
    static WizWorkQueue* workQueue = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        workQueue =  [WizGlobalData shareInstanceFor:[WizWorkQueue class] category:WizDocDisplayModelWorkQueue];
    });
    return workQueue;
}
...
- (id) init
{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(removeAllWorkObject) name:WizErrorMessageUserPasswordInvalid object:nil];
    }
    return self;
}
