...
//            DDLogCWarn(@"content: %@",currentNodeContent);
      ...
      while (attribute)
        {
          ...
          if (attributeName)
            {
//                DDLogCWarn(@"Attribute Name Set: %@",attributeName);
...
NSArray *PerformXPathQuery(xmlDocPtr doc, NSString *query)
{
  ...
  if(xpathCtx == NULL)
    {
      DDLogCWarn(@"Unable to create XPath context.");
      return nil;
    }
  ...
  if(xpathObj == NULL) {
    DDLogCWarn(@"Unable to evaluate XPath.");
