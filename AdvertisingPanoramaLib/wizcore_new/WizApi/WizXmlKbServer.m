...
- (BOOL) getObjectList:(id<WizObject>)list first:(int64_t)first count:(int64_t)count  methodKey:(NSString*)methodKey
{
    ...
    [postParams setObject:@(count) forKey:@"count"];
    [postParams setObject:@(first) forKey:@"version"];
...
- (BOOL) getDocumentData:(NSString*)guid info:(BOOL)info param:(BOOL)param retObject:(WizDocument*)retObject
{
    NSMutableDictionary* postParams = [NSMutableDictionary dictionary];
    [postParams setObject:guid forKey:@"document_guid"];
    postParams[@"document_info"] = BoolTOString(info);
    postParams[@"document_param"] = BoolTOString(param);
    return [self callXmlRpc:postParams methodKey:SyncMethod_DocumentGetData retObj:retObject];
}
