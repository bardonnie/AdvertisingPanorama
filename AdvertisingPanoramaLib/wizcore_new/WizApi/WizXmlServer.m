...
{
    if (IsReachableInternet() && CurrentNetWorkStatues() == ReachableViaWWAN) {
        return YES;
    }
    return NO;
}

NSString* const SyncMethod_SetReadStatus                = @"accounts.setReadStatus";
NSString* const SyncMethod_GetMessages                  = @"accounts.getMessages";
...
- (BOOL) callXmlRpc:(XMLRPCRequest*)request retObj:(id<WizObject>)ret
{
    @autoreleasepool {
        ...
        if (response == nil) {
            self.lastError = [NSError errorWithDomain:@"WizError" code:-1 userInfo:[NSDictionary dictionaryWithObject: NSLocalizedString(@"Network is error!", nil) forKey:NSLocalizedDescriptionKey]];
            return NO;
        }
...
- (void) addCommontParams:(NSMutableDictionary*)postParams
{
    ...
    [postParams setObject:version forKey:@"client_version"];
